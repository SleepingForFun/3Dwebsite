...
let query = "SELECT product_id, productname, package_id, prodcolor, category_id, saleprice, status FROM product";
// execute query
...
router.get('/:recordid/show', function(req, res, next) {
    let query = "SELECT product_id, productname,prodimage, description, ingredients, weightdetail, healthwarn, prodcolor, package_id,category_id, saleprice, status FROM product WHERE product_id = " + req.params.recordid;
    // execute query
    db.query(query, (err, result) => {
    if (err) {
    console.log(err);
...
router.get('/:recordid/edit', function(req, res, next) {
    let query = "SELECT product_id, productname,prodimage, description, ingredients, weightdetail, healthwarn, prodcolor, package_id,category_id, saleprice, status FROM product WHERE product_id = "
    + req.params.recordid;
    // execute query
    db.query(query, (err, result) => {
    if (err) {
...
router.post('/save', function(req, res, next) {
    let updatequery = "UPDATE product SET productname = ?, prodimage = ?, description = ?, ingredients = ?, weightdetail = ?, healthwarn = ?, prodcolor = ?, package_id = ?, category_id = ?, saleprice = ?, status = ? WHERE product_id = " + req.body.product_id;
